...
export default function MobileVehicleList({ vehicles }: { vehicles: Vehicle[] }) {
  ...
  const [active, setActive] = useState(
    mode === 'chauffeured' ? 'chauffeur' : mode === 'selfdrive' ? 'selfdrive' : 'all'
  )

  useEffect(() => {
    if (mode === 'chauffeured') setActive('chauffeur')
    else if (mode === 'selfdrive') setActive('selfdrive')
  }, [mode])
