...
type TileId = 'ms' | 'gc' | 'smtp' | 'push' | 'crazytel'
...
function GcPanel({ gcConfigured, gcConnected: initConnected, gcConnectedEmail: initEmail }: {
  gcConfigured: boolean; gcConnected: boolean; gcConnectedEmail: string
}) {
  const [connected, setConnected] = useState(initConnected)
  const [email, setEmail] = useState(initEmail)
  const [saving, setSaving] = useState(false)
  ...
  return (
    <div className="bg-white border border-border rounded-xl overflow-hidden">
      <SectionHeader title="Google Calendar" description="Sync bookings to Google Calendar for staff scheduling visibility." />
      <div className="px-5 py-4 space-y-4">
        {msg && <p className={`text-[13px] rounded-[6px] px-3 py-2 ${msg.type === 'success' ? 'text-success bg-success-bg border border-success/30' : 'text-red-600 bg-red-50 border border-red-200'}`}>{msg.text}</p>}
        {!gcConfigured ? (
          <div className="bg-yellow-50 border border-yellow-200 rounded-[6px] px-4 py-3 text-[13px] text-yellow-700">
          ...
          <div className="space-y-3">
            <div className="bg-success-bg border border-success/30 rounded-[6px] px-4 py-3 flex items-center justify-between gap-4">
              <div>
                <p className="text-[13px] text-success font-semibold">Connected</p>
                <p className="text-[12px] text-ink-3 mt-0.5">Syncing as: <span className="font-medium text-ink">{email}</span></p>
              </div>
              <button onClick={handleDisconnect} disabled={saving}
                className="text-[12px] font-semibold text-red-600 border border-red-200 bg-red-50 hover:bg-red-100 px-3 py-1.5 rounded-[5px] transition-colors disabled:opacity-50 whitespace-nowrap">
                {saving ? 'Disconnecting…' : 'Disconnect'}
            ...
            <p className="text-[12px] text-ink-4">Colour-coded: yellow = pending, green = confirmed, red = cancelled, navy = completed, purple = enquiry.</p>
          </div>
        ) : (
          <div className="space-y-3">
            <p className="text-[13px] text-ink-3">Connect your Google account to sync bookings to Google Calendar automatically.</p>
            <a href="/api/admin/settings/gc-auth"
...
function ConnectionsFormInner({
  ...
  useEffect(() => {
    if (searchParams.get('ms') === 'connected' || (searchParams.get('error') && !searchParams.get('gc'))) setSelected('ms')
    else if (searchParams.get('gc') === 'connected') setSelected('gc')
  }, []) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const tiles: Array<{ id: TileId; label: string; description: string; status: TileStatus; icon: React.ReactNode }> = [
    {
      id: 'ms',
      label: 'Microsoft 365',
      description: 'Email via Outlook OAuth',
    ...
    },
    {
      ...
      description: 'Sync bookings to Calendar',
      status: !gcConfigured ? 'not_set' : gcConnected ? 'connected' : 'not_set',
      icon: (
  ...
  return (
    <div className="space-y-5">
      ...
      {selected === 'gc' && (
        <GcPanel gcConfigured={gcConfigured} gcConnected={gcConnected} gcConnectedEmail={gcConnectedEmail} />
      )}
      {selected === 'smtp' && (
